...
/** Get the mask if there is one */
    ...
    typename OptionType::ValueType fixed_initial_affine_filename = this->m_Parser->GetOption( "fixed-image-initial-affine" )->GetValue();
    ...
    this->m_RegistrationOptimizer->SetFixedImageAffineTransform( this->m_TransformationModel->GetFixedImageAffineTransform());
...
::ReadImagesAndMetrics()
{
    ...
    typedef ImageFileReader<ImageType> ReaderType;
    bool useHistMatch = this->m_Parser->template Convert<bool>( this->m_Parser->GetOption( "use-Histogram-Matching" )->GetValue() );
...
//                whichMetric == "JTB"
            ...
            if ( isMetricPointSetBased )
              ...
              std::cout << "Metric " << i << ": " << " Point-set " << whichMetric <<  " n-params " <<  option->GetNumberOfParameters( i )  << std::endl;
              /**
...
//         option->SetDescription( "this mask -- defined in the 'fixed' image space defines the region of interest over which the registration is computed ==> above 0.1 means inside mask \n\t\t==> continuous values in range [0.1 , 1.0] effect optimization like a probability.  \n\t\t==> values > 1 are treated as = 1.0 " );
    ...
    if (true)
      ...
      std::string weightDescription("The metric weights are relative to the weights on the N other metrics passed to ANTS --- N is unlimited.  So, the weight, w_i on the i^{th} metric will be  w_i=w_i/ ( sum_i  w_i ).");
      ...
      std::string msqDescription( "MSQ/mean-squares/MeanSquares -- demons-like, radius > 0 uses moving image gradient in metric deriv." );
...
//      std::string jtbDescription( "JTB/jensen-tsallis-bspline/JensenTsallisBSpline" );
    ...
    if (true)
    {
        ...
        option->SetDescription( "The name for the output - a prefix or a name+type : e.g.  -o OUT or  -o OUT.nii  or -o OUT.mha " );
        this->m_Parser->AddOption( option );
    }
    ...
    if (true)
      ...
      option->SetDescription(" enable to use weighted sum of all metric terms for convergence computation. By default, only the first metric is used");
...
//    }
    ...
    if (true)
    {
        ...
        option->SetDescription( "reference space for using the input file as the initial affine parameter for the fixed image " );
        this->m_Parser->AddOption( option );
    }
    if (true)
    {
...
// compute the Euclidean length of the diffeomorphism and write to an image -- OutputNamethick.nii.gz -- This is a Beta version of thickness computation -- Not Full-Blown DiReCT , Das, 2009, Neuroimage ---  syn with time is the only model that can be used with this option " );
        this->m_Parser->AddOption( option );
    }
    ...
    if (true)
    {
        ...
        option->SetShortName( 'G' );
        option->SetDescription( " true / false -- if true, SyN is faster but loses some accuracy wrt inverse-identity constraint, see Avants MIA 2008.");
    ...
    if (true) {
         ...
         option->SetDescription( "MI: mutual information (default), MSQ: mean square error, SSD, CC: Normalized correlation, CCH: Histogram-based correlation coefficient (not recommended), GD: gradient difference (not recommended) " );
    ...
    if (true) {
         ...
         option->SetDescription( "option of gradient descent in affine transformation:  maximum_step_length x relaxation_factor x minimum_step_length x translation_scales ");
    ...
    if (true) {
        ...
        option->SetDescription( "ignore the apparently unmatched origins (when use-rotation-header is false and the rotation matrix is identity: true (default) / false" );
        std::string nitdefault=std::string("false");
        option->AddValue(nitdefault);
        this->m_Parser->AddOption( option );
    }

...
void
AverageTimeImages( typename TImageIn::Pointer image_in ,  typename TImageOut::Pointer image_avg , std::vector<unsigned int> timelist )
{
...
int ants_motion( itk::ants::CommandLineParser *parser )
{
  ...
  for( int currentStage = numberOfStages - 1; currentStage >= 0; currentStage-- )
    ...
    if( std::strcmp( whichMetric.c_str(), "cc" ) == 0 )
      ...
      typedef itk::MattesMutualInformationImageToImageMetricv4<FixedImageType, FixedImageType> MutualInformationMetricType;
    ...
    double small_step=0;
    for (unsigned int i=0;i<ImageDimension;i++) small_step+=fixed_time_slice->GetSpacing()[i]*fixed_time_slice->GetSpacing()[i];
    ...
    if( std::strcmp( whichTransform.c_str(), "affine" ) == 0 )
      ...
      std::string filename = outputPrefix + std::string("TimeSlice") + ants_moco_to_string<unsigned int>(timedim) + std::string( "Affine.txt" );
	...
	param_values(timedim,i+2)=affineRegistration->GetOutput()->Get()->GetParameters()[i];
      ...
      std::string filename = outputPrefix + std::string("TimeSlice") + ants_moco_to_string<unsigned int>(timedim) + std::string( "Rigid.txt" );
      typedef itk::TransformFileWriter TransformWriterType;
      typename TransformWriterType::Pointer transformWriter = TransformWriterType::New();
    ...
    for(  vfIter2.GoToBegin(); !vfIter2.IsAtEnd(); ++vfIter2 )
      {
        ...
        for (unsigned int xx=0; xx<ImageDimension; xx++) ind[xx]=vfIter2.GetIndex()[xx];
...
static void InitializeCommandLineOptions( itk::ants::CommandLineParser *parser )
{
  ...
  {
  std::string description = std::string( "use a fixed reference image instead of the neighor in the time series." );
  OptionType::Pointer option = OptionType::New();
  ...
  option->SetUsageOption( 2, "GaussianDisplacementField[gradientStep,updateFieldSigmaInPhysicalSpace,totalFieldSigmaInPhysicalSpace]" );
  option->SetDescription( description );
  parser->AddOption( option );
  }
...
RcppExport SEXP antsMotionCorr( SEXP r_args )
try
{
  ...
  std::string commandDescription = std::string( "ants_moco = motion correction.  This program is a user-level " ) +
    ...
    std::string( " Specialized for 4D time series data: fixed image is 3D, moving image should be the 4D time series. ")+
  ...
  std::cout << std::endl << "Running " << argv[0] << "  for " << dimension << "-dimensional images." << std::endl << std::endl;

...
int antsApplyTransforms( itk::ants::CommandLineParser *parser )
{
  ...
  if( inputOption && inputOption->GetNumberOfValues() > 0 )
    {
    Rcpp::Rcout << "Input object: " << inputOption->GetValue() << std::endl;
    ...
    if( outputOption->GetNumberOfParameters( 0 ) > 1 &&
      ...
      {
      Rcpp::Rcout << "An input image is required." << std::endl;
      return EXIT_FAILURE;
      }
    }
  ...
  if( referenceOption && referenceOption->GetNumberOfValues() > 0 )
    {
    Rcpp::Rcout << "Reference image: " << referenceOption->GetValue() << std::endl;
    ...
    {
    Rcpp::Rcout << "Error:  No reference image specified." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if( transformOption && transformOption->GetNumberOfValues() > 0 )
        ...
        try
          ...
          if( transformOption->GetNumberOfParameters( n ) == 0 )
            ...
            if( ( transformOption->GetNumberOfParameters( n ) > 1 ) &&
              ...
              if( !transform )
                {
                Rcpp::Rcout << "Inverse does not exist for " << transformName
                  << std::endl;
                return EXIT_FAILURE;
                }
          ...
          {
          Rcpp::Rcout << "Transform reader for " <<
    ...
    Rcpp::Rcout << "The composite transform is comprised of the following transforms "
      << "(in order): " << std::endl;
    for( unsigned int n = 0; n < transformNames.size(); n++ )
      {
      Rcpp::Rcout << "  " << n+1 << ". " << transformNames[n] << " (type = "
  ...
  if( interpolationOption && interpolationOption->GetNumberOfValues() > 0 )
    ...
    if( !std::strcmp( whichInterpolator.c_str(), "linear" ) )
      ...
      {
      Rcpp::Rcout << "Error:  Unrecognized interpolation option." << std::endl;
      return EXIT_FAILURE;
      }
    }
  Rcpp::Rcout << "Interpolation type: " <<
  ...
  Rcpp::Rcout << "Default pixel value: " <<
  ...
  if( outputOption && outputOption->GetNumberOfValues() > 0 )
    {
    if( outputOption->GetNumberOfParameters( 0 ) > 1 && parser->Convert<unsigned int>( outputOption->GetParameter( 0, 1 ) ) != 0 )
      {
      Rcpp::Rcout << "Output composite transform displacement field: " << outputOption->GetParameter( 0, 0 ) << std::endl;
      ...
      Rcpp::Rcout << "Output warped image: " << outputFileName << std::endl;
...
try
{
  ...
  if( inputOption && inputOption->GetNumberOfValues() > 0 )
    ...
    {
    Rcpp::Rcout << "No reference image was specified." << std::endl;
    return Rcpp::wrap( EXIT_FAILURE ) ;
    }
